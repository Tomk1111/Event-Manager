...
public class main
{
	...
	public static void main(String [] args)
	{
	 ...
	 /**
	   * Restores previously generated objects on program start from their respective files
	   * Files are scanned and these variables found are used to recreate the objects.
	   **/	
	...
	/**
	  ...
	  *
	  * @param filename filename is the name of the file being searched.
	  * @param str String str is the string we are searching for in the chosen file
	  * @param pos int pos is the position of the searched string in a line in the file split at ","
	  **/
	...
	/**
	  ...
	  *
	  * @param email takes string to check if it is a valid email addresss
	  *	@return returns a true boolean if the email matches the pattern regex. Otherwise 
	  * it returns false.
	...
	public static boolean loginMethod(String email, String password) 
	{
	 ...
	 /**
	   * This method checks if a given date is in the correct format to be parsed to a LocalDate.
	   *
	   * @param date String date is passed to check if it is in valid LocalDate format according to the given DateTimeFormatter
	   * @return if the string can be parsed to LocalDate, it returns true. Else it catches the exception and returns false/
	   **/		
		public static boolean isValidDate(String date)throws IOException
	{
	 ...
	 /**
	   * Creates a new facility according to admin preference.
	   * Admin can name the facility, set the price per hour and decommission it until a chosen date.
	   * Scans facility arrayList and checks if any facilities exist with the same name and if so asks admin to choose another name.
	   * Calls isValidDate() method if the admin wishes to decommission it until a further date.
	   **/		
